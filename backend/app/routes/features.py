...
def summarize(document: str, db: Session = Depends(get_db)):
    ...
    try:
        # Larger budget than a chat turn: the JSON summary must come back whole,
        # and gemini-2.5-flash's thinking tokens also draw from this cap.
        raw = generate_reply([{"role": "user", "content": prompt}], max_tokens=2048)
    except LLMError:
